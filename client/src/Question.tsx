...
type QuestionProps = IQuestion & { onChange: (index: string, value: string, type: ChangeType, code: string) => void; index: string }
...
export const Question = (props: QuestionProps) => {
    ...
    return <div>
        <div style={{ display: 'flex', alignItems: isSubQuestions ? 'center' : 'start', flexDirection: 'row', gap: 10 }}>
        ...
        <div >
            {props.subQuestions?.map((sq: IQuestion, subQuestionIndex: number) => (<div > <Question {...sq} index={`${props.index}_${subQuestionIndex}`} onChange={props.onChange} />  </div>))}
        </div>
    </div>;
}